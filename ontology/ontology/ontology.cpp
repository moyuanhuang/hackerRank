...
struct TrieNode{
    ...
    TrieNode(): is_leaf(false), num_question(0)
...
private:
    Tree FlatTreeHelper(string str){
        ...
        unordered_map<string, bool> has_added;
        for(size_t i = 0; i < str.length(); i++){
            ...
            if(pos < str.length()){
                ...
                if(cur_topic == ")"){
                    ...
                    string father = st.top();
                    tree.AddNode(father, children);
                    has_added[father] = true;
